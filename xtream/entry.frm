...
If Rs2.State = adStateOpen Then Rs2.Close
    Rs2.Open "select * from entrytbl where doc_no='" & doctxt.Text & "'", Cn, adOpenDynamic, adLockOptimistic
...
If KeyAscii = 9 Or KeyAscii = 13 Then
    If Rs2.State = adStateOpen Then Rs2.Close
    Rs2.Open "select * from entrytbl where doc_no='" & doctxt.Text & "'", Cn, adOpenStatic, adLockReadOnly
    If Rs2.RecordCount > 0 And Not IsNull(Rs2("invoice_no")) Then
    MsgBox "This is already inculded in Invoice " & Rs2("invoice_no"), vbInformation + vbOKOnly
    '>>>Disable all keys
    Command1.Enabled = False
    Command2.Enabled = False
    Command3.Enabled = False
    Command4.Enabled = False
    Command5.Enabled = False
    ElseIf Rs2.RecordCount > 0 And IsNull(Rs2("invoice_no")) Then
    Call ClearText
    blnAuto = True
